...
export const JPEG = {
  ...
  JPEGEncoder: function (a) {
    ...
    function N(a) {
      for (let b = a[0], c = a[1] - 1; c >= 0; )
        (b & (1 << c) && (r |= 1 << s),
          c--,
          s--,
          s < 0 && (r == 255 ? (O(255), O(0)) : O(r), (s = 7), (r = 0)));
    }
    function O(a) {
      q.push(w[a]);
    }
    ...
    function T() {
      let a, b;
      for (P(65499), P(132), O(0), a = 0; a < 64; a++) O(e[a]);
      for (O(1), b = 0; b < 64; b++) O(f[b]);
    }
    function U() {
      let a, b, c, d, e, f, g, h;
      for (P(65476), P(418), O(0), a = 0; a < 16; a++) O(A[a + 1]);
      for (b = 0; b <= 11; b++) O(B[b]);
      for (O(16), c = 0; c < 16; c++) O(C[c + 1]);
      for (d = 0; d <= 161; d++) O(D[d]);
      for (O(1), e = 0; e < 16; e++) O(E[e + 1]);
      for (f = 0; f <= 11; f++) O(F[f]);
      for (O(17), g = 0; g < 16; g++) O(G[g + 1]);
      for (h = 0; h <= 161; h++) O(H[h]);
    }
    function V() {
      (P(65498),
        P(12),
    ...
    function W(a, b, c, d, e) {
      ...
      for (
        q = p[0] - c,
          c = p[0],
          q == 0 ? N(d[0]) : ((h = 32767 + q), N(d[n[h]]), N(m[h])),
          r = 63;
        r > 0 && p[r] == 0;
        r--
      );
    ...
    function X() {
      ...
      for (b = 0; b < 256; b++) w[b] = a(b);
    }
    function Y(a) {
      if ((a <= 0 && (a = 1), a > 100 && (a = 100), y != a)) {
        let b = 0;
        ((b = a < 50 ? Math.floor(5e3 / a) : Math.floor(200 - 2 * a)),
          I(b),
          (y = a),
    ...
    (Math.round,
      ...
      (this.encode = function (a, b) {
        ...
        for (
          ...
          for (y = 0; p > y; ) {
            for (D = p * z + y, E = D, F = -1, G = 0, H = 0; H < 64; H++)
              ((G = H >> 3),
                (F = 4 * (7 & H)),
                (E = D + G * p + F),
                z + G >= o && (E -= p * (z + 1 + G - o)),
                ...
                (C = m[E++]),
                (t[H] =
                  ((x[A] + x[(B + 256) >> 0] + x[(C + 512) >> 0]) >> 16) - 128),
                (u[H] =
                  ((x[(A + 768) >> 0] +
                    x[(B + 1024) >> 0] +
                    x[(C + 1280) >> 0]) >>
