...
const manifest = JSON.parse(
  fs.readFileSync(path.resolve(__dirname, './dist-ssr/client/.vite/manifest.json'), 'utf-8')
)
...
server.all(/^(.*)$/, async (req, res) => {
  try {
    const template = fs.readFileSync(path.resolve(__dirname, './dist-ssr/client/indexSSR.html'), 'utf-8')
